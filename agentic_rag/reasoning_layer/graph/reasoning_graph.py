...
def guardrail_node(state):
    if hallucination.check(state["query"], state["answer"], state["docs"]):
...
def retry_node(state):
    state["retry"] = retry_agent.decide(state["reasoning"])
    return state
...
builder.add_conditional_edges("retry", lambda s: END)
